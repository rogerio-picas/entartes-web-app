...
export default function NovaMarcacaoModal({ onClose, onSuccess, initialSlot }) {
  ...
  const horasPossiveis = (() => {
    ...
    while (new Date(cursor.getTime() + duracao * 60000) <= fim) {
      const h = cursor.toISOString().substring(11, 19) // "HH:MM:SS"
      const hhmm = h.substring(0, 5) // "HH:MM"
      // Só incluir horas dentro do intervalo permitido (08:30–21:30)
      // e que a sessão termine até às 21:30
      const endTime = new Date(cursor.getTime() + duracao * 60000)
      const endHHMM = endTime.toISOString().substring(11, 16)
      if (hhmm >= HORA_MIN && endHHMM <= HORA_MAX) {
        horas.push(h)
      }
