...
export default function HomeAdmin() {
    ...
    async function handleConfirm(id, id_sala) {
  setLoadingAction(id)
  try {
    // Update state to confirmed
    await aulasService.updateEstado(id, aulasService.ESTADOS.CONFIRMADA)

    // If sala was selected, update the marcacao with the sala too
    if (id_sala) {
      await fetch(`/api/aulas/${id}/sala`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json',
          Authorization: `Bearer ${localStorage.getItem('token')}` },
        body: JSON.stringify({ id_sala })
      }).catch(() => {}) // best-effort — sala endpoint may not exist yet
    }
